...
public class BaseRenderer implements GLSurfaceView.Renderer {
    ...
    public static int logicWidth = 0;
    public static int logicHeight = 0;
    public boolean isInitiated = false;
    protected IDrawable myDrawable;

    protected final FPS mFPS = new FPS();
    ...
    /**
     * @param pDrawable
     *            the IDrawable to be draw the IDrawable should be initialized. if the IDrawable is set before the renderer is Created, the IDrawable
     *            can be initialize at the end of creating function.
     */
    ...
    public void onSurfaceCreated(GL10 pGl, EGLConfig config) {
        Log.d("[MyRenderer]", "onSurfaceCreated:" + Thread.currentThread().toString() + "/" + Thread.currentThread().getId());
        gl = pGl;
    ...
    public static void loadIdentity(GL10 gl) {
        gl.glLoadIdentity();
        gl.glTranslatef(-logicWidth / 2, -logicHeight / 2, 0f);
    }

    /**
     * set the clear color, this can be used as background color a,r,g,b are float from 0 to 1
     * 
     * @param gl
     * @param a
     * @param r
